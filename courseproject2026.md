...
## Submission of 1st deliverable 25 February 2026

## Submission of 2nd deliverable 07 April 2026
...
of ID, e.g. SSN/SIN/driving licence, and the date of their registration into our system. For employees
of the hotels, we need to store their full name, address and SSN/SIN. The employees may have
various roles/positions in a hotel. Every hotel needs to have a manager.


The customers can search for room availability and book rooms through the online application for
specific dates. When they check-in the hotel, their room _booking_ is transformed to room _renting_ and
they can also pay for this renting. The employee that does the check-in for a customer is responsible
for transforming the room booking to renting. Customer may present themselves physically at a hotel
without a booking and directly ask for a room. In this case the employee at the hotel can do the renting
of the room right away without a prior booking.
We need to store in the database the history of both bookings and rentings (archives), but we do not
need to store the history of payments. Information about an old (archived) room booking/renting must
exist in the database, even if information about the room itself or the customer does not exist in the
database anymore. We should be able to delete from our database hotel chains, hotels and rooms. We
cannot have in the database information about a room without having in the database the information
about the corresponding hotel (i.e. the hotel in which the room belongs too). In the same way, we
cannot have in the database information about a hotel without having in the database the information
about the corresponding hotel chain (i.e. the hotel chain in which the hotel belongs too).

You are requested to do the following:

1a. (1 0 %) **ER diagram** : Create the ER diagram that corresponds to the above description.
...
for this purpose.
2 e. ( 5 %) **Database indexes** : Implement at least 3 indexes on the relations of your database and
...
database.
2 f. ( 5 %) **Database views** : Implement 2 views (in SQL). View 1: the first view is the number of
available rooms per area. View 2: the second view is the aggregated capacity of all the rooms
of a specific hotel. You are welcome to implement more views of your choice.
2 g. (30%) **Web application** : Design and implement an appropriate User Interface, through which
...
1. A report that includes the following:
    ...
    c. A list with the DDLs that create your database (i.e. all the SQL statements that create
       your database)
